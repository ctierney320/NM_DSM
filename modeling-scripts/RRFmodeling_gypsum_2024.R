#### Ranger Random Forest Modeling Gypsum
#### Charlotte Tierney
...
## Pedon point data
pedons <- read.csv(paste(savefolder,"processed/pedons_ALL_est.csv",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/PEDONS_gypsum.csv") #must be csv, rds requires more manipulation
...
property <- "Gypsum"    #common name for file saving
prop <- "gyp_FINAL" #column name in pedon table
tier <- "gyp_tier" #column name for property tier info
version <- "Fullqt24TEST" #version name for file saving #full dataset, quantiles, transformed, 2024

trans <- "log" #transformation type for table, around line 155

#### 1.e GRAB GYPSUM DATA *if needed
WantedGypsum <- c("pedon_id","utm_east","utm_north","topdepth_cm","botdepth_cm",
                  "gyp_FINAL","gyp_tier") #
ALL_gypsum <- subset(pedons, select = WantedGypsum)
...
ALL_gypsum <- as.data.frame(ALL_gypsum)

saveRDS(ALL_gypsum,paste(savefolder,"processed/","Gypsum_Pedons.rds",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/PEDONS_xx.rds")
write.csv(ALL_gypsum,paste(savefolder,"processed/","Gypsum_Pedons.csv",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/PEDONS_xx.csv")
...
# d <- 0
# pts.ext <- readRDS(paste(savefolder,"final/",property,"_ExtractedPtsObj_",version,".rds",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/Gypsum_Fullq_ExtractedPts.rds")
...
# read in extracted points object if you have it
# pts.ext <- readRDS(paste(savefolder,"final/",property,"_ExtractedPtsObj_",version,".rds",sep=""))
...
# pts.ext <- readRDS(paste(savefolder,"final/",property,"_ExtractedPts_",version,".rds",sep=""))#pts.ext <- readRDS("data/final/Gypsum_ExtractedPts_Fullqt24TEST.rds")
...
# pts.ext <- pts.ext <- readRDS(paste(savefolder, "final/Gypsum_ExtractedPts_Fullqt24.rds",sep="")) 

pts.ext$gyp_FINAL <- as.numeric(pts.ext$gyp_FINAL)
# pts.ext <- na.omit(pts.ext$gyp_FINAL) #CHECK FOR NAs THOUGH
...
#separate by depth
depths <- c(0,5,15,30,60,100,150) #(0,5,
# d <- 0 #for tests
...
for(d in depths){
  ...
  pts.extd <- pts.extd[c(covs.list,prop,tier)]## create a specific list of dependent variable and covariate names to use 
  pts.extd <- na.omit(pts.extd)# Remove any record with NA's (in any column - be careful)
  
  #Transform data
  pts.extd$transformed <- log(pts.extd$gyp_FINAL + 1) #gypsum, ec
  ...
  # varrange <- as.numeric(quantile(pts.extd$gyp_FINAL, probs=c(0.975), na.rm=T) - quantile(pts.extd$gyp_FINAL, probs=c(0.025),na.rm=T))
  ...
  ################ Predictions 
  ## this step takes about 24 hours PER DEPTH on WM1701
  ...
  saveRDS(rangerQmodel, paste(savefolder,"processed/",property,"_",d,"cm","_RangerModelObj_",version,".rds",sep=""))
  ...
  # pts.extd <- readRDS(paste(savefolder,"final/","ExtractedPoints_0cm_Gypsum_Aug5NMqt.rds",sep=""))
  ...
  # predh_bt <- readRDS(paste(savefolder,"final/","Predh_bt_object_0cm_Gypsum_Aug5NMqt.rds",sep=""))
  # predl_bt <- readRDS(paste(savefolder,"final/","Predl_bt_object_0cm_Gypsum_Aug5NMqt.rds",sep=""))
   
   
  ##RUN UNCERTAINTY:
  pred.uncert.data <- (predh_bt - predl_bt)/varrange #*100                  #####CHECK TRANSFORMATION (use predh_bt and predl_bt if so)
  ...
  # #if not transformed:
  # pts.extcvm$prop_t <- pts.extcvm$gyp_FINAL ## UPDATE: transform if needed else just create new version of prop #log(   +1) or sqrt()
  ...
  pts.extpcv$prop <- pts.extpcv$gyp_FINAL                                     ## UPDATE HERE FOR EVERY PROPERTY, TRANSFORM TYPE
  pts.extpcv$pcvpred_bt <- (exp(pts.extpcv$pcvpred))-1  #^2   (exp(x))-1     ## UPDATE EVERY TIME #BACKTRANSFORM HERE
  ...
  pts.extpcv$prop_bt <- (exp(pts.extpcv$prop_t))-1 #^2 # UPDATE: backtransform if necessary. Used for PICP and to characterize backtransformation bias
  ...
  # new:
  proponly <- na.omit(as.numeric(pts.extd$gyp_FINAL))
  ...
  # stats <- table(pts.ext$gyp_tier)
  #this is so messy, sorry everyone
  stats.indv <- table(pts.extd$gyp_tier)
