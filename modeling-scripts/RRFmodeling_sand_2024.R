#### Ranger Random Forest Modeling Sand
#### Charlotte Tierney
...
property <- "Sand"    #common name for file saving
prop <- "sand_FINAL" #column name in pedon table
tier <- "sand_tier" #column name for property tier info
version <- "Fullq24TEST" #version name for file saving #full dataset, quantiles, not transformed, 2024

trans <- "none" #transformation type for table, around line 170

#### 1.e GRAB SAND DATA *if needed
WantedSand <- c("pedon_id","utm_east","utm_north","topdepth_cm","botdepth_cm",
                 "sand_FINAL","sand_tier") #
ALL_SAND <- subset(pedons, select = WantedSand)
ALL_SAND <- ALL_SAND %>% drop_na(c("utm_east","utm_north","sand_FINAL"))

coordinates(ALL_SAND) <- ~utm_east+utm_north #assign coordinates
proj4string(ALL_SAND) <- CRS(projargs=ProjectCRS)
plot(ProjExtent)
plot(ALL_SAND, add=TRUE)


ALL_SAND <- as.data.frame(ALL_SAND)

saveRDS(ALL_SAND,paste(savefolder,"processed/","Sand_Pedons.rds",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PEDONS_Sand.rds")
write.csv(ALL_SAND,paste(savefolder,"processed/","Sand_Pedons.csv",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PEDONS_Sand.csv")
...
# pedons <- readRDS(paste(savefolder,"processed/","Sand_Pedons.rds",sep="")) #if you didn't just run this
...
# i <- 102 #test
starttime <- Sys.time()
for (i in length(covs)){ #length(covs)){ #3){ #103:106
...
# pts.ext <- readRDS(paste(savefolder,"final/",property,"_ExtractedPts_",version,".rds",sep=""))#pts.ext <- readRDS("data/final/Sand_ExtractedPts_Fullqt24TEST.rds")
...
# pts.ext <- pts.ext <- readRDS(paste(savefolder, "final/Sand_ExtractedPts_Fullqt24.rds",sep="")) #

pts.ext$sand_FINAL <- as.numeric(pts.ext$sand_FINAL)
# pts.ext <- na.omit(pts.ext$sand_FINAL) #CHECK FOR NAs THOUGH
...
for(d in depths){
  ...
  #Transform data
  # pts.extd$transformed <- log(pts.extd$sand_FINAL + 1) #gypsum, ec
  # pts.extd$transformed <- (exp(pts.extd$sand_FINAL))-1 #ucrb caco3
  # pts.extd$transformed <- sqrt(pts.extd$sand_FINAL) #eh caco3
  # pts.extd$transformed <- (pts.extd$sand_FINAL)^2 #deffo not
  ...
  varrange <- as.numeric(quantile(pts.extd$sand_FINAL, probs=c(0.975), na.rm=T) - quantile(pts.extd$sand_FINAL, probs=c(0.025),na.rm=T))
  ...
  # pts.extd <- readRDS(paste(savefolder,"final/","ExtractedPoints_0cm_Sand_Aug5NMqt.rds",sep=""))
  ...
  # predh_bt <- readRDS(paste(savefolder,"final/","Predh_bt_object_0cm_Sand_Aug5NMqt.rds",sep=""))
  # predl_bt <- readRDS(paste(savefolder,"final/","Predl_bt_object_0cm_Sand_Aug5NMqt.rds",sep=""))
  ...
  #if not transformed:
  pts.extcvm$prop_t <- pts.extcvm$sand_FINAL ## UPDATE: transform if needed else just create new version of prop #log(   +1) or sqrt()
  ...
  pts.extpcv$prop <- pts.extpcv$sand_FINAL                                     ## UPDATE HERE FOR EVERY PROPERTY, TRANSFORM TYPE
  ...
  # new:
  proponly <- na.omit(as.numeric(pts.extd$sand_FINAL))
  ...
  # stats <- table(pts.ext$sand_tier)
  #this is so messy, sorry everyone
  stats.indv <- table(pts.extd2$sand_tier)
