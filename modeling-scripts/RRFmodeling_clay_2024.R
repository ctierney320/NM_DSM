#### Ranger Random Forest Modeling Clay
#### Charlotte Tierney
...
## Pedon point data
pedons <- read.csv(paste(savefolder,"processed/pedons_ALL_est.csv",sep="")) #must be csv, rds requires more manipulation
...
snaprast <- raster("D:/CPT_work/NM_DSM/COVS_compressed/ca_mosaic.tif") #grab snap raster #backed up on U:/ drive
...
property <- "Clay"    #common name for file saving
prop <- "clay_FINAL" #column name in pedon table
tier <- "clay_tier" #column name for property tier info
version <- "Fullq24TEST" #version name for file saving #full dataset, quantiles, transformed, 2024

trans <- "none" #transformation type for table, around line 170

# #### 1.e GRAB CLAY DATA *if needed
# WantedClay <- c("pedon_id","utm_east","utm_north","topdepth_cm","botdepth_cm",
#                  "clay_FINAL","clay_tier") #
# ALL_CLAY <- subset(pedons, select = WantedClay)
# ALL_CLAY <- ALL_CLAY %>% drop_na(c("utm_east","utm_north","clay_FINAL"))
# 
# coordinates(ALL_CLAY) <- ~utm_east+utm_north #assign coordinates
# proj4string(ALL_CLAY) <- CRS(projargs=ProjectCRS)
# plot(ProjExtent)
# plot(ALL_CLAY, add=TRUE)
# 
# 
# ALL_CLAY <- as.data.frame(ALL_CLAY)
# 
# saveRDS(ALL_CLAY,paste(savefolder,"processed/","Clay_Pedons.rds",sep=""))#
# write.csv(ALL_CLAY,paste(savefolder,"processed/","Clay_Pedons.csv",sep=""))#
# 
# pedons <- ALL_CLAY
# rm(ALL_CLAY)
...
# d <- 0
# pts.ext <- readRDS(paste(savefolder,"final/",property,"_ExtractedPtsObj_",version,".rds",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/Clay_Fullq_ExtractedPts.rds")
# pts.extd <- readRDS(paste(savefolder,"final/",property,"_",d,"cm","_ExtractedPtsObj_",version,".rds",sep=""))
...
pedons <- readRDS(paste(savefolder,"processed/","Clay_Pedons.rds",sep="")) #if needed
...
# i <- 102 #test
starttime <- Sys.time()
for (i in length(covs)){ #length(covs)){ #3){ #1:101 #103:106
...
# # #1/18/24 note: ran wc2.0_bio_30s_15 (cov 102) from the G drive bc the D drive version was glitching in GDAL
#   rast <- terra::rast(covs2[102])
#   pts.ext.indv <- terra::extract(rast, pedons.v)
#   file.name <- covs2[102] #rast@ptr[["filenames"]] #get file name for covariate column
#   file.name <- gsub(".tif","", file.name)
#   file.name <- gsub(".*/","",file.name)
#   names(pts.ext.indv)[2] <- file.name
#   pts.ext <<- merge(pts.ext,pts.ext.indv, by="ID") #c("utm_east","utm_north"))
## 4/5/24 note: RIP the D: drive
...
# pts.ext <- readRDS(paste(savefolder,"final/",property,"_ExtractedPts_",version,".rds",sep=""))#pts.ext <- readRDS("data/final/Clay_ExtractedPts_Fullqt24TEST.rds")
...
# pts.ext <- pts.ext <- readRDS(paste(savefolder, "final/Clay_ExtractedPts_Fullqt24.rds",sep="")) 

pts.ext$clay_FINAL <- as.numeric(pts.ext$clay_FINAL)
# pts.ext <- na.omit(pts.ext$clay_FINAL) #CHECK FOR NAs THOUGH
...
for(d in depths){
  ...
  #Transform data
  # pts.extd$transformed <- log(pts.extd$clay_FINAL + 1) #gypsum, ec
  # pts.extd$transformed <- (exp(pts.extd$clay_FINAL))-1 #ucrb caco3
  # pts.extd$transformed <- sqrt(pts.extd$clay_FINAL) #eh caco3
  # pts.extd$transformed <- (pts.extd$clay_FINAL)^2 #deffo not
  ...
  varrange <- as.numeric(quantile(pts.extd$clay_FINAL, probs=c(0.975), na.rm=T) - quantile(pts.extd$clay_FINAL, probs=c(0.025),na.rm=T))
  ...
  # pts.extd <- readRDS(paste(savefolder,"final/","ExtractedPoints_0cm_Clay_Aug5NMqt.rds",sep=""))
  ...
  # predh_bt <- readRDS(paste(savefolder,"final/","Predh_bt_object_0cm_Clay_Aug5NMqt.rds",sep=""))
  # predl_bt <- readRDS(paste(savefolder,"final/","Predl_bt_object_0cm_Clay_Aug5NMqt.rds",sep=""))
  ...
  #if not transformed: #pretend you're transforming to keep the tables happy
  pts.extcvm$prop_t <- pts.extcvm$clay_FINAL ## UPDATE: transform if needed else just create new version of prop #log(   +1) or sqrt()
  ...
  ## Back transformed: create pcvpred_bt even if not transformed for cv.depth function
  pts.extpcv$prop <- pts.extpcv$clay_FINAL                                     ## UPDATE HERE FOR EVERY PROPERTY, TRANSFORM TYPE
  ...
  #don't backtransform:
  pts.extpcv$prop_bt <- pts.extpcv$prop_t #pretend you're backtransforming to keep the tables happy
  ...
  # new:
  proponly <- na.omit(as.numeric(pts.extd$clay_FINAL))
  ...
  # stats <- table(pts.ext$clay_tier)
  #this is so messy, sorry everyone
  stats.indv <- table(pts.extd2$clay_tier)
...
write.table(stats.all, paste(savefolder,"final/",property,"_ALL_ModelCV_stats_",version,".csv",
                               sep=""), sep = "\t", row.names = FALSE)
