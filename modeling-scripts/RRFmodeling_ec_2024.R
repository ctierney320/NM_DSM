#### Ranger Random Forest Modeling EC
#### Charlotte Tierney
...
covfolder <- "D:/CPT_work/NM_DSM/COVS_compressed/" #"G:/DSM_TWN/covar_sets/NM_compressed/" #CHANGE #NOT on git
...
property <- "EC"    #common name for file saving
prop <- "ec_FINAL" #column name in pedon table
tier <- "ec_tier" #column name for property tier info
...
#### 1.e isolate EC property data, if needed
WantedEC <- c("pedon_id","utm_east","utm_north","topdepth_cm","botdepth_cm",
              "ec_FINAL","ec_tier") #
ALL_EC <- subset(pedons, select = WantedEC)
ALL_EC <- ALL_EC %>% drop_na(c("utm_east","utm_north","ec_FINAL"))

coordinates(ALL_EC) <- ~utm_east+utm_north #assign coordinates
proj4string(ALL_EC) <- ProjectCRS #old: CRS(projargs=ProjectCRS)
plot(ProjExtent)
plot(ALL_EC, add=TRUE)

ALL_EC <- as.data.frame(ALL_EC)

saveRDS(ALL_EC,paste(savefolder,"processed/","EC_Pedons.rds",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/PEDONS_EC.rds")
write.csv(ALL_EC,paste(savefolder,"processed/","EC_Pedons.csv",sep=""))#"C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/PEDONS_EC.csv")
...
# d <- 0
# pts.ext <- readRDS(paste(final,"final/",property,"_ExtractedPts_",version,".rds",sep=""))
# pts.extd <- readRDS(paste(final,"final/",property,"_",d,"cm","_ExtractedPtsObj_",version,".rds",sep=""))
...
pedons <- readRDS(paste(savefolder,"processed/","EC_Pedons.rds",sep="")) #if you didn't just run them
...
# pts.ext <- readRDS("data/final/EC_ExtractedPts_Fullqt24TEST.rds")
...
# pts.ext <- pts.ext <- readRDS(paste(savefolder, "processed/EC_ExtractedPts_Fullqt24.rds",sep="")) #C:/Users/cptierney/OneDrive - DOI/2022_OneDrive/PedonData/EC_Fullq_ExtractedPts.rds")

pts.ext$ec_FINAL <- as.numeric(pts.ext$ec_FINAL)
# pts.ext <- na.omit(pts.ext$ec_FINAL) #CHECK FOR NAs THOUGH
...
for(d in depths){
  ...
  #Transform data
  pts.extd$transformed <- log(pts.extd$ec_FINAL + 1) #gypsum, ec
  ...
  # varrange <- as.numeric(quantile(pts.extd$ec_FINAL, probs=c(0.975), na.rm=T) - quantile(pts.extd$ec_FINAL, probs=c(0.025),na.rm=T))
  ...
  # pts.extd <- readRDS(paste(savefolder,"final/","ExtractedPoints_**.rds",sep=""))
  ...
  # predh_bt <- readRDS(paste(savefolder,"final/","**.rds",sep=""))
  # predl_bt <- readRDS(paste(savefolder,"final/","**.rds",sep=""))
  
  
  ##RUN UNCERTAINTY:
  pred.uncert.data <- (predh - predl)/varrange #*100                  #####CHECK TRANSFORMATION (use predh_bt and predl_bt if so)
  ...
  # #if not transformed:
  # pts.extcvm$prop_t <- pts.extcvm$ec_FINAL ## UPDATE: transform if needed else just create new version of prop #log(   +1) or sqrt()
  ...
  pts.extpcv$prop <- pts.extpcv$ec_FINAL                                     ## UPDATE HERE FOR EVERY PROPERTY, TRANSFORM TYPE
  ...
  # new:
  proponly <- na.omit(as.numeric(pts.extd$ec_FINAL))
  ...
  # stats <- table(pts.ext$ec_tier)
  #this is so messy, sorry everyone
  stats.indv <- table(pts.extd2$ec_tier)
